...
class DatabaseManager:
        ...
        try:
            query = """
                ...
                (session_id, symbol, start_date, end_date, timeframe, status,
                 ...
                 NOW(), error_count, last_error, created_at, updated_at)
    ...
    async def log_historical_market_data(self, symbol: str, timeframe: str, open_time,
                                       close_time, open_price: float, high_price: float,
                                       low_price: float, close_price: float, volume: float,
                                       quote_volume: float = None, trade_count: int = None,
                                       taker_buy_volume: float = None, taker_buy_quote: float = None,
                                       rsi: float = None, macd: float = None, macd_signal: float = None,
                                       macd_histogram: float = None, ema_short: float = None,
                                       ema_long: float = None, bb_upper: float = None,
                                       bb_middle: float = None, bb_lower: float = None,
                                       atr: float = None, data_quality: float = 1.0) -> bool:
        """Log historical market data with technical indicators"""
        try:
            await self.log_market_data({
                ...
                'volume': volume,
                'quote_volume': quote_volume or (volume * close_price),
                'trade_count': trade_count or 100,
...
def get_global_database_manager():
...
# MarketDataRecord class for compatibility
class MarketDataRecord:
    ...
    def log_signal(self, signal_data):
        ...
        try:
            query = """
                ...
                (symbol, signal_type, action, confidence, reasoning, created_at)
                VALUES (%s, %s, %s, %s, %s, %s)
            """
    ...
    async def log_market_data(self, market_data_dict):
        """Log market data using dictionary input"""
        try:
            query = """
                INSERT INTO market_data_temporal 
                (symbol, timeframe, open_time, close_time, open_price, high_price, low_price, 
                 close_price, volume, quote_volume, trade_count, data_quality)
            ...
            self.execute_query(query, (
                ...
                market_data_dict.get('timeframe'),
                market_data_dict.get('open_time'),
                market_data_dict.get('close_time'),
                market_data_dict.get('open_price'),
                market_data_dict.get('high_price'),
                market_data_dict.get('low_price'),
                market_data_dict.get('close_price'),
                market_data_dict.get('volume'),
                market_data_dict.get('quote_volume'),
                market_data_dict.get('trade_count'),
                market_data_dict.get('data_quality', 1.0)
            ))
            return True
    ...
    async def log_market_data(self, market_data_dict):
        """Log market data - compatible interface"""
        try:
            query = """
                INSERT INTO market_data_temporal 
                (symbol, timeframe, open_time, close_time, open_price, high_price, low_price, 
            ...
            self.execute_query(query, (
                market_data_dict.get('symbol'),
                market_data_dict.get('timeframe', '1h'),
                market_data_dict.get('open_time'),
                market_data_dict.get('close_time'),
                market_data_dict.get('open_price'),
                market_data_dict.get('high_price'),
